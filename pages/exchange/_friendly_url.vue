          ...
          <form class="form">
            ...
            <div class="exchangeform-step-2">
              ...
              <div
                ...
                v-html="parseSeoText(currentRoute.seo.instruction)"
              />
...
export default {
  ...
  methods: {
    ...
    parseSeoText(text) {
      if (text) {
        text = text
          .replace(/\[\[inNameCurrency\]\]/g, this.currentRoute.from.name)
          .replace(/\[\[outNameCurrency\]\]/g, this.currentRoute.to.name)
          .replace(/\[\[inSymbol\]\]/g, this.currentRoute.from.symbol)
          .replace(/\[\[outSymbol\]\]/g, this.currentRoute.to.symbol);
      }
      text = this.randomTextParser(text);
      return text;
    },

    randomTextParser(text) {
      if (text) {
